...
class J5Client extends DigsEmitter {
  constructor() {
    ...
    if (!(process.env.DIGS_ID && process.env.DIGS_FSQ_DIR)) {
      throw new Error('Invalid environment');
    }

    let env = this._env = _.pick(process.env, function(value, key) {
    ...
    let baseTopic = this._baseTopic =
      [env.DIGS_NAMESPACE, env.DIGS_PROJECT, clientId].join('/');
