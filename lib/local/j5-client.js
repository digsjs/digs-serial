...
class J5Client extends DigsEmitter {
  ...
  start() {
    return ascoltatori.buildAsync(this._ascoltatoriOpts)
      ...
      .then(function(ascoltatore) {
        ...
        this.subscribe(`${this._clientId}/+`, function(message, topic) {
      ...
      .then(function(board) {
        this._localDevice = board;
        debug(`${this}: Successfully started board "${board.id}"`);
        return this.publish(`${this.id}/ready`, {
          id: board.id,
          port: board.port
        });
      })
  ...
  subscribe() {
    return this._ascoltatore.subscribe.apply(this._ascoltatore, arguments);
  }
  ...
  _startBoard() {
    ...
    let opts = _.defaults(_.omit(this._boardOpts || {}, 'name'), {
      id: _.uniqueId('digs-local-board-')
    });
