...
let debug = require('debug')('digs:digs-local:localdevice');
...
class LocalDevice extends DigsEmitter {
  ...
  fork(attempt) {
    ...
    this.subscribe(`${this.id}-local/+`, function(data, topic) {
      let event = topic.split('/').slice(1).join('/');
      debug(`Emitting event "${event}" with data`, data);
    ...
    return this.when('ready', 'error')
      ...
      .then(function(data) {
        this.port = data.port;
        debug(`Board "${this.id}" is ready on port ${this.port}`);
  ...
  subscribe(topic, callback) {
    return this._ascoltatore.subscribe(topic, function(data) {
      return callback.call(this, data, topic);
    }.bind(this));
  }

  publish(topic, data) {
    topic = `${this.id}-local/${topic}`;
    debug(`Publishing topic "${topic}" with data`, data);
